...
import { defaultPlayerData } from 'ReplicatedStorage/shared/state/PlayersState'
...
export = () => {
  describe('state', () => {
    ...
    it('should update player inventory', () => {
      ...
      const playerState = selectTestPlayer(state)
      expect(playerState?.inventory.Conveyor).to.equal(
        (defaultPlayerData.inventory.Conveyor ?? 0) + 13,
      )
    })
    ...
    it('should move from player inventory and container', () => {
      ...
      const playerState = selectTestPlayer(state)
      expect(playerState?.inventory?.Conveyor).to.equal(
        (defaultPlayerData.inventory.Conveyor ?? 0) + 2,
      )
      ...
      expect(playerState2?.inventory?.Conveyor).to.equal(
        (defaultPlayerData.inventory.Conveyor ?? 0) + 11,
      )
      expect(playerState2?.containers?.foobar?.Conveyor).to.equal(2)

      expect(
        selectTestPlayer(
          store.movePlayerItem(testUserId, 'foobar', 'Conveyor', -200),
        ),
      ...
      expect(playerState3?.inventory?.Conveyor).to.equal(
        (defaultPlayerData.inventory.Conveyor ?? 0) + 13,
      )
